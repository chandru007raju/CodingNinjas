...
// BinaryTreeNode<int> * root; // 2 4 3 1 6 8 9 7 10 ---> input -> 9
...
void pairSum (BinaryTreeNode<int>* root,int sum)
{
    ...
    BinaryTreeNode<int>* originalRoot = root;

    int temp = root->data;
    bool ispresent = searching(originalRoot,sum - temp);
    root->data = INT_MIN;
...
void tooArray(BinaryTreeNode<int> * root, vector<int> &arr)
{
    if(root == NULL){
        return NULL;
    }
    ...
    arr.push_back(root->data);
    tooArray(root->left,arr);
    tooArray(root->right,arr);
}
