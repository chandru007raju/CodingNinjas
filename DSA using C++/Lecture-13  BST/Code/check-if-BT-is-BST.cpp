...
ReturnBst isBSTriplet(BinaryTreeNode<int>* root)
{
    if(root == NULL){
        ReturnBst ans;
        ans.isBiSearch = true;
    ...
    ReturnBst leftans = isBSTriplet(root->left);
    ReturnBst rightans = isBSTriplet(root->right);
    ...
    bool isBinary = root->data > maximun && root->data <= minimun && leftans.isBiSearch && rightans.isBiSearch;

    ReturnBst final;
    final.minimum = minimum;
    final.maximum = maximum;
    final.isBiSearch = isBinary;
...
bool isBST(BinaryTreeNode <int>* root)
{
    return isBSTriplet(root).isBiSearch;
}
